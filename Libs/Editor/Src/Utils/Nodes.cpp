...
namespace Rift::Nodes
{
	...
	{
		const bool anyUIElementHovered = gNodes->hoveredNodeId.IsValid()
	...
	void BoxSelectorUpdateSelection(EditorContext& editor, Rect boxRect)
	{
		...
		for (AST::Id id : editor.nodes)
		{
			NodeData& node = editor.nodes.Get(id);
			if (boxRect.Overlaps(node.rect))
			{
				editor.selectedNodeIds.push_back(nodeIdx);
			}
		}
	}
	...
	for (i32 linkIdx = 0; linkIdx < editor.Links.Pool.Size(); ++linkIdx)
	{
		if (editor.Links.InUse.IsSet(linkIdx))
		{
			...
			const v2 end = GetScreenSpacePinCoordinates(nodeEndRect, PinType::Input, inputPin.rect);
...
v2 SnapOrigi32oGrid(v2 origin)
{
	if ((gNodes->style.Flags & StyleFlags_GridSnapping)
	    || ((gNodes->style.Flags & StyleFlags_GridSnappingOnRelease) && gNodes->leftMouseReleased))
	{
...
OptionalIndex FindDuplicateLink(const EditorContext& editor, const PinIdx pinA, const PinIdx pinB)
{
...
bool CanLinkSnapToPin(
    const EditorContext& editor, PinIdx originPin, PinIdx targetPin, OptionalIndex duplicateLink)
{
...
void TranslateSelectedNodes(EditorContext& editor)
{
	...
	{
		const v2 origin = SnapOrigi32oGrid(gNodes->mousePosition - gNodes->CanvasOriginScreenSpace
		...
		for (i32 i = 0; i < editor.selectedNodeIds.size(); ++i)
		{
			...
			const i32 nodeIdx = editor.selectedNodeIds[i];
...
void UpdateBoxSelection(EditorContext& editor)
{
	...
	gNodes->CanvasDrawList->AddRect(boxRect.min, boxRect.max, boxSelectorOutline.ToPackedABGR());

	if (gNodes->leftMouseReleased)
	{
		ImVector<AST::Id>& depthStack        = editor.nodes.depthOrder;
...
void UpdateLinkCreation(EditorContext& editor)
{
	...
	// endpoint to it
	const v2 endPos = shouldSnap ? GetScreenSpacePinCoordinates(editor, gNodes->HoveredPinIdx.type,
	                      editor.GetPinData(gNodes->HoveredPinIdx))
	                             : gNodes->mousePosition;

	const CubicBezier cubicBezier =
	    MakeCubicBezier(startPos, endPos, startIdx.type, gNodes->style.linkLineSegmentsPerLength);
...
{
	const ImVector<i32>& depthStack = editor.nodes.depthOrder;
...
OptionalIndex ResolveHoveredNode(const TArray<AST::Id>& depthStack)
{
	...
	{
		return OptionalIndex(gNodes->NodeIdsOverlappingWithMouse[0]);
	}

	i32 largestDepthIdx = -1;
	i32 nodeIdxOnTop    = -1;

	for (int nodeIdx : gNodes->NodeIdsOverlappingWithMouse)
	{
	...
	switch (pin.Shape)
	{
		...
		case PinShape_CircleFilled: {
			gNodes->CanvasDrawList->AddCircleFilled(
			    pinPos, gNodes->style.PinCircleRadius, pinColor.ToPackedABGR(), circleNumSegments);
		}
		break;
		case PinShape_Quad: {
		...
		case PinShape_QuadFilled: {
			...
			gNodes->CanvasDrawList->AddQuadFilled(pinPos + offset.topLeft,
			    pinPos + offset.bottomLeft, pinPos + offset.bottomRight, pinPos + offset.topRight,
			    pinColor.ToPackedABGR());
		}
		break;
		case PinShape_Triangle: {
			const TriangleOffsets offset =
			    CalculateTriangleOffsets(gNodes->style.PinTriangleSideLength);
			gNodes->CanvasDrawList->AddTriangle(pinPos + offset.topLeft, pinPos + offset.bottomLeft,
			    pinPos + offset.right, pinColor.ToPackedABGR(),
...
void StyleColorsClassic()
{
	...
	gNodes->style.colors[ColorVar_MiniMapNodeBackgroundSelected] =
	    gNodes->style.colors[ColorVar_MiniMapNodeBackgroundHovered];
	gNodes->style.colors[ColorVar_MiniMapNodeBackgroundSelected] = Color::FromRGB(200, 200, 240);
	gNodes->style.colors[ColorVar_MiniMapNodeOutline] = Color::FromRGB(200, 200, 200, 100);
	gNodes->style.colors[ColorVar_MiniMapLink]        = gNodes->style.colors[ColorVar_Link];
	gNodes->style.colors[ColorVar_MiniMapLinkSelected] =
	    gNodes->style.colors[ColorVar_LinkSelected];
...
void EndNodeEditor()
{
	    ...
	    && IsMouseInCanvas() && !editor.miniMap.IsHovered())
	{
		...
		if (!gNodes->hoveredNodeId.IsValid())
		{
	...
	for (NodeData& node : editor.nodes)
	{
	...
	if (!editor.miniMap.IsHovered())
	{
		...
		else if (gNodes->leftMouseClicked || gNodes->leftMouseReleased || gNodes->altMouseClicked
		         || gNodes->altMouseScrollDelta != 0.f)
		{
	...
	for (AST::Id nodeId : nodes)
	{
		if (ast.IsValid(nodeId))
		{
			auto& node = ast.Get<NodeData>();
			node.inputs.clear();
			node.outputs.clear();
		}
		else
...
void EndNode()
{
	...
	{
		gNodes->NodeIdsOverlappingWithMouse.push_back(gNodes->CurrentNodeIdx);
	}
}
...
bool IsNodeHovered(Id* const nodeId)
{
	...
	const bool isHovered = gNodes->hoveredNodeId.IsValid();
	if (isHovered)
	{
		const EditorContext& editor = GetEditorContext();
		*nodeId                     = editor.nodes.Pool[gNodes->hoveredNodeId.Value()].id;
	}
	return isHovered;
}
...
{
	EditorContext& editor = GetEditorContext();
	ClearObjectSelection(editor.nodes, editor.selectedNodeIds, nodeId);
}
...
{
	EditorContext& editor = GetEditorContext();
	SelectObject(editor.nodes, editor.selectedNodeIds, nodeId);
}
...
{
	EditorContext& editor = GetEditorContext();
	return IsObjectSelected(editor.nodes, editor.selectedNodeIds, nodeId);
}
bool IsNodeSelectedByIdx(i32 nodeIdx)
{
	EditorContext& editor = GetEditorContext();
	return IsObjectSelectedByIdx(editor.nodes, editor.selectedNodeIds, nodeIdx);
}
...
bool IsLinkCreated(Id& outputPinId, Id& inputPinId, bool* createdFromSnap)
{
	...
	if ((gNodes->UIState & UIState_LinkCreated) != 0)
	{
		...
		if (createdFromSnap)
		{
			*createdFromSnap = editor.clickInteraction.type == ClickInteractionType_LinkCreation;
		}
		return true;
	}
	return false;
