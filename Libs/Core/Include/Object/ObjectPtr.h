...
public:
	...
	Type* operator*() const
	{
		return static_cast<Type*>(BaseGlobalPtr::operator*());
	}
	Type* operator->() const
	{
		return static_cast<Type*>(BaseGlobalPtr::operator->());
	...
	template <typename Type2>
	bool operator==(const GlobalPtr<Type2>& other) const
	{
		return **this == *other;
	}
	template <typename Type2>
...
public:
	BaseGlobalPtr* __GetGlobal() const
	{
		return const_cast<BaseGlobalPtr*>(globalPtr);
	}
