...
Rotator Quat::ToRotator() const
{
	...
	// but that isn't the case for us, so I went through different testing, and finally found the
	// case where both of world lives happily.
	...
	else
	{
		...
		RotatorFromQuat.Roll() =
			Math::Atan2(-2.f * (w * x + y * z), (1.f - 2.f * (Math::Square(x) + Math::Square(y)))) *
			Math::RADTODEG;
	}
